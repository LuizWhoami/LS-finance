...
class FinanceDashboardView(LoginRequiredMixin, PermissionRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        total_subscriptions = subscriptions.filter(
        ...
        revenue_subscriptions = subscriptions.filter(
            created_at__date__gte=start_date,
            created_at__date__lte=end_date,
            is_active=True
        ).aggregate(
        ...
        # Totais
        total_revenue = revenue_appointments + revenue_subscriptions + total_income
        total_profit = profit_appointments + revenue_subscriptions + net_profit
        
        context['total_revenue'] = total_revenue
        context['total_profit'] = total_profit
        
        # Métricas adicionais
        total_income_aux = total_income + revenue_appointments + revenue_subscriptions
        ...
        context['other_income'] = total_income - revenue_appointments - revenue_subscriptions
        ...
        while current_date <= end_date:
            ...
            total_day = day_income + day_appointments
            daily_data.append({
...
class BarberProfitReportView(LoginRequiredMixin, PermissionRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        total_rating = sum([b['rating'] for b in barber_data if b['rating'] > 0])
        rating_count = len([b for b in barber_data if b['rating'] > 0])
        context['avg_rating'] = total_rating / rating_count if rating_count > 0 else 0
